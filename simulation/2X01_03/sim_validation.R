    ...
    for (rho in rho_set) {
        for (p in p_set) {
            for (sigma in sigma_set) {
                fn_out = paste0("validation_", design, "_rho", rho, "_p", p, "_sigma", sigma, ".RData")
                ...
                for (njob in 1:NJOB) {
                    ...
                    while (nsim <= NSIM) {
                        ...
                        res = summary(lm(simY~simX+simX2))
						index = NSIM*njob-NSIM+nsim
                        results[index,c(1,3)] = coef(res)[2:3,1]
                        results[index,c(2,4)] = coef(res)[2:3,2]
                        nsim = nsim+1
                    }    
                }
