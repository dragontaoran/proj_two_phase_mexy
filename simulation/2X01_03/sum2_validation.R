...
for (nsieve in nsieve_set) {
    for (hn in hn_set) {
        for (design in design_set) {
            fn_out = paste0("sum2_validation_nsieve", nsieve, "_hn", hn, "_", design, ".tab")

            
            sink(fn_out)
            cat("\t\t\t\t\t\t\t\tRE\t\t\n")
            cat("$\\tau$\t$r$\t$p$\tCovariate\tBias\tSE\tSEE\tCP\tMBE\tCCE\tSMLE\n")
            sink()
            ...
            for (sigma in sigma_set) {
                for (rho in rho_set) {
                    for (p in p_set) {
						...
						prefix = paste0("validation_", design, "_rho", rho, "_p", p, "_sigma", sigma)
                        ...
                        res.sd = sd(results[,1])
						res[i,9] = (res.sd/as.numeric(res[i,6]))^2
                        
                        res.sd = sd(results[,3])
                        res[i+1,9] = (res.sd/as.numeric(res[i+1,6]))^2
                        
                        prefix = paste0("CC2000_", design, "_rho", rho, "_p", p, "_sigma", sigma)
                        load(paste0("results/", prefix, ".RData"))
                        
                        res.sd = sd(results[,1])
						res[i,10] = (res.sd/as.numeric(res[i,6]))^2
                        
                        res.sd = sd(results[,3])
                        res[i+1,10] = (res.sd/as.numeric(res[i+1,6]))^2
                        
						prefix = paste0("nsieve", nsieve, "_hn", hn, "_", design, "_sigma", sigma, "_rho", rho, "_p", p)
                        load(paste0("results/", prefix, ".RData"))

                        res.sd = sd(results[,1])
						res[i,11] = (res.sd/as.numeric(res[i,6]))^2
                        
                        res.sd = sd(results[,3])
                        res[i+1,11] = (res.sd/as.numeric(res[i+1,6]))^2
