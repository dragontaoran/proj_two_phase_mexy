...
echo "#SBATCH --time=00-03:00:00  # Wall Clock time (dd-hh:mm:ss) [max of 14 days]" >> tmp.slurm
echo "#SBATCH --output=${dir}/CC2000.slog  # output and error messages go to this file" >> tmp.slurm
echo "#SBATCH --job-name=CC # job name" >> tmp.slurm
echo "#SBATCH --constraint=\"sandybridge|haswell|skylake\"" >> tmp.slurm
echo "" >> tmp.slurm
echo "echo \"SLURM_JOBID: \" \$SLURM_JOBID" >> tmp.slurm
echo "" >> tmp.slurm
echo "Rscript sim_CC2000.R ${dir}" >> tmp.slurm
