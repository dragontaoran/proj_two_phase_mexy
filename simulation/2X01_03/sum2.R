...
for (nsieve in nsieve_set) {
    for (hn in hn_set) {
        for (design in design_set) {
            ...
            sink(fn_out)
            cat("\t\t\t\tSY\t\t\t\t\t\tCC\t\t\t\t\t\tSMLE\t\t\t\n")
            cat("$\\tau$\t$r$\t$p$\tCovariate\tBias\tSE\tSEE\tCP\tRE\t\tBias\tSE\tSEE\tCP\tRE\t\tBias\tSE\tSEE\tCP\n")
            sink()
            
            res = matrix(NA, nrow=nrow, ncol=20)
            ...
            for (sigma in sigma_set) {
                for (rho in rho_set) {
                    for (p in p_set) {
                        ...
                        prefix = paste0("CC2000_", design, "_rho", rho, "_p", p, "_sigma", sigma)
                        load(paste0("results/", prefix, ".RData"))
                        ...
                        res[i+1,11] = mean(results[,3])-gamma
                        res[i+1,12] = sd(results[,3])
                        res[i+1,13] = mean(results[,4])
                        res[i+1,14] = mean((results[,3]-z975*results[,4] <= gamma) & (results[,3]+z975*results[,4] >= gamma))
                        
						prefix = paste0("nsieve", nsieve, "_hn", hn, "_", design, "_sigma", sigma, "_rho", rho, "_p", p)
                        load(paste0("results/", prefix, ".RData"))

                        res[i,17] = mean(results[,1])-beta
                        res[i,18] = sd(results[,1])
                        res[i,19] = mean(results[,2])
                        res[i,20] = mean((results[,1]-z975*results[,2] <= beta) & (results[,1]+z975*results[,2] >= beta))
                        res[i,9] = (as.numeric(res[i,18])/as.numeric(res[i,6]))^2
						res[i,15] = (as.numeric(res[i,18])/as.numeric(res[i,12]))^2
                        
                        res[i+1,17] = mean(results[,3])-gamma
                        res[i+1,18] = sd(results[,3])
                        res[i+1,19] = mean(results[,4])
                        res[i+1,20] = mean((results[,3]-z975*results[,4] <= gamma) & (results[,3]+z975*results[,4] >= gamma))
                        res[i+1,9] = (as.numeric(res[i+1,18])/as.numeric(res[i+1,6]))^2
						res[i+1,15] = (as.numeric(res[i+1,18])/as.numeric(res[i+1,12]))^2
