...
for (nsieve in nsieve_set) {
    for (hn in hn_set) {
        ...
        res_srs = read.table(paste0("../validity05/", fn_out), sep="\t", as.is=TRUE, skip=2)
        
        sink(fn_out)
        cat("\t\tSY\t\t\t\t\tMLE\t\t\t\t\n")
        cat("rho\tp\tBias\tSE\tSEE\tCP\t\tBias\tSE\tSEE\tCP\tRE\n")
        sink()
        
        res = matrix(NA, nrow=nrow, ncol=12)
        ...
        for (rho in rho_set) {
            for (p in p_set) {
                ...
                res[i,12] = (res_srs[i,9]/res[i,9])^2
