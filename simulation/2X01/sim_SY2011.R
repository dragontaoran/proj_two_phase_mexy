    ...
    for (rho in rho_set) {
        for (p in p_set) {
            for (sigma in sigma_set) {
                fn_out = paste0("SY2011_", design, "_rho", rho, "_p", p, "_sigma", sigma, ".RData")
                ...
                for (njob in 1:NJOB) {
                    ...
                    while (nsim <= NSIM) {
                        ...
                        res = audit.correct.cov.vdependent(simY_tilde, simX_tilde, simX2, simX, simY, c(1-pX2, pX2), simX2, poss.cat=unique(simX2))
                        index = NSIM*njob-NSIM+nsim
                        results[index,c(1,3)] = res$CorrectEst
                        results[index,c(2,4)] = sqrt(diag(res$VarCorrectEst))
                        nsim = nsim+1
                    }    
                }
