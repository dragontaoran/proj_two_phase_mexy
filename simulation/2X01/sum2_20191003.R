...
for (nsieve in nsieve_set) {
    for (hn in hn_set) {
        for (design in design_set) {
            fn_out = paste0("sum2_nsieve", nsieve, "_hn", hn, "_", design, "_20191003.tab")
            
            sink(fn_out)
            cat("\t\t\t\tSY\t\t\t\tCC\t\t\t\tSMLE\t\t\t\n")
            cat("$\\tau$\t$r$\t$p$\tCovariate\tBias\tSE\tRE\t\tBias\tSE\tRE\t\tBias\tSE\tSEE\tCP\n")
            sink()
            ...
            for (sigma in sigma_set) {
                for (rho in rho_set) {
                    for (p in p_set) {
                        ...
                        res[i+1,9] = mean(results[,3])-gamma
                        res[i+1,10] = sd(results[,3])
                        ...
                        res[i,7] = (as.numeric(res[i,14])/as.numeric(res[i,6]))^2
						res[i,11] = (as.numeric(res[i,14])/as.numeric(res[i,10]))^2
                        
                        res[i+1,13] = mean(results[,3])-gamma
                        res[i+1,14] = sd(results[,3])
                        res[i+1,15] = mean(results[,4])
                        res[i+1,16] = mean((results[,3]-z975*results[,4] <= gamma) & (results[,3]+z975*results[,4] >= gamma))
                        res[i+1,7] = (as.numeric(res[i+1,14])/as.numeric(res[i+1,6]))^2
						res[i+1,11] = (as.numeric(res[i+1,14])/as.numeric(res[i+1,10]))^2
